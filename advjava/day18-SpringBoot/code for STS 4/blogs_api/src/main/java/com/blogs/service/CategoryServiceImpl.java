...
public class CategoryServiceImpl implements CategoryService {
	...
	public ApiResponse addNewCategory(CategoryReqDTO dto) {
		/*
		 * dev steps 1. dto -> entity (using model mapper) 2. save entity 3. send back
		 * dto - api resp
		 */
		Category categoryEntity = modelMapper.map(dto, Category.class);
		Category persistentCategory = categoryDao.save(categoryEntity);
		return new ApiResponse("Added new category with ID=" + persistentCategory.getId());
	}

	@Override
	public CategoryRespDTO getCategoryDetails(Long catId) {
		Category categoryEntity = categoryDao.findById(catId)
	...
	@Override
	public ApiResponse updateCategory(Long categoryId, CategoryReqDTO dto) {
